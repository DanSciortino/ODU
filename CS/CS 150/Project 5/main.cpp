...
int main()
{
    ...
    while( add_record )
    {
        ...
        cout << "Enter point #" << ( record_list->num_nodes + 1 ) << ": ";

        //read the values into to_add
        cin >> to_add -> x >> to_add -> y;

        //Add the node to the  Linked List
        addNode( record_list, to_add );

        if (record_list->num_nodes > 2 &&  record_list->num_nodes < 5)
        {
    ...
    //Ouput the list
    printHeading( cout, "Summary: Points & Perimeter", W_WIDTH);
    ...
    while( list_iterator != NULL)
    {
        ...
        cout << fixed << setprecision(2) << "Point " << recnum << ":  (  " << (list_iterator->data)->x << ",  " << (list_iterator->data)->x << " )"<< endl;
        recnum++;
...
list_iterator = record_list->head;
    ...
    if(record_list->num_nodes == 3)
 {

        perimeter = (list_iterator->data)->x * (list_iterator->next->data)->x;

    }

        if(record_list->num_nodes == 4)
    {
        perimeter = 5*5;
